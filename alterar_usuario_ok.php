...
mysql_select_db("empresa_db_",$connect_altera_senha);
...
<?php
//conecção php com sql server maxdata....
/*$connect = odbc_connect("DRIVER={SQL Server}; SERVER=192.168.1.100;  DATABASE=banco;", "tabela","senha");
...
mysql_select_db("banco-db",$connect_altera_senha);

//consulta banco para compara senha no if abaixo
$sql_consulta_senha = "select id,usuario from usuarios where id = '$idusu'";
...
{ 
$senha_atual_banco =$linha_pesquisa['usuario'];
} 
...
if ($senha_atual_formulario == $senha_atual_banco) {
    ...
    echo 'Usuario Alterada com sucesso!!! Redirecionando para pagina inicial...';
		...
		//consulta banco para compara senha no if abaixo
$sql_altera_senha = "UPDATE usuarios SET `usuario` = '$nova_senha_formulario' WHERE id = '$idusu'";
		//executa consulta fisicamente
